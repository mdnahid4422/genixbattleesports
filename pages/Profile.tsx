...
import { Camera, Loader2, Save, LogOut, CheckCircle2, Shield, Target, Trophy, Award, Zap, Star } from 'lucide-react';
import { db, doc, updateDoc, auth, signOut } from '../firebase';
          ...
          <div className="flex flex-col md:flex-row items-center md:items-end gap-6 mb-12">
            <div className="relative group">
              <div className="w-40 h-40 rounded-[48px] border-4 border-[#060608] bg-[#060608] overflow-hidden shadow-2xl relative">
                ...
                <img src={user.photoURL || `https://api.dicebear.com/7.x/avataaars/svg?seed=${user.uid}`} className="w-full h-full object-cover" alt="Profile" />
                <button 
            ...
            <div className="text-center md:text-left flex-grow">
               <div className="flex items-center justify-center md:justify-start gap-3">
                 <h2 className="font-orbitron text-4xl font-black italic text-white uppercase tracking-tighter">{user.fullName}</h2>
                 <CheckCircle2 size={24} className="text-blue-400" />
               </div>
               <p className="text-gray-500 font-bold uppercase tracking-[0.3em] text-[10px] mt-1 italic">Operative Protocol {user.uid.slice(0, 8)}</p>
            </div>
             ...
             <div className="space-y-8">
                ...
                <div className="space-y-4">
                   ...
                   <BadgeItem 
                     ...
                     icon={<Shield size={16}/>} 
                     label={user.role} 
                     color={user.role === 'owner' ? 'from-red-600 to-orange-600' : user.role === 'admin' ? 'from-purple-600 to-blue-600' : 'from-gray-600 to-slate-700'} 
                   />
                   ...
                   <div className="grid grid-cols-2 gap-3">
                      {user.specialBadges && user.specialBadges.length > 0 ? user.specialBadges.map(b => (
...
const SpecialBadgeItem = ({ label }: { label: SpecialBadge }) => {
  const getColors = () => {
    switch(label) {
